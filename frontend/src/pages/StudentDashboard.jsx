  ...
  return (
    <div>
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        <p className="page-subtitle">Connect with fellow NIT JSR students</p>
      </div>
...
export default function StudentDashboard({ user, setUser }) {
  ...
  // Profile completion check — show banner if profile is incomplete and user hasn't dismissed it this session
  useEffect(() => {
    if (!user) return;
    const dismissed = sessionStorage.getItem('profile_banner_dismissed');
    if (dismissed) return;
    const isIncomplete = !user.department || !user.graduationYear || !(user.skills?.length > 0);
            ...
            {showProfileBanner && (
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  ...
                  <div>
                    <div style={{ fontWeight: 700, fontSize: '0.9375rem', marginBottom: '0.125rem' }}>Complete your profile to unlock full features!</div>
                    <div style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)' }}>Add your department, skills & graduation year to get personalized job matches and referrals.</div>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexShrink: 0 }}>
                  <button className="btn btn-primary btn-sm" onClick={() => handleTabChange('profile')}>Complete Profile →</button>
                  <button onClick={() => { setShowProfileBanner(false); sessionStorage.setItem('profile_banner_dismissed', '1'); }}
